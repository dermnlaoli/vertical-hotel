...
class DataInspector(models.Model):
    ...
    snap_qc_hitung = fields.One2many('data.snap.qc.line', 'snap_qc_ids')
    snap_qc_denah = fields.One2many('data.mesin.urw', 'snap_denah')
    no_mesin = fields.Many2one('data.mesin.urw', string="No Mesin")
    ...
    hitung_keterangan = fields.Integer(string="Keterangan Count", compute='_compute_hitung_keterangan', store=True)

    @api.depends('snap_qc_line.keterangan')
    def _compute_hitung_keterangan(self):
        for inspector in self:
            inspector.hitung_keterangan = sum(1 for line in inspector.snap_qc_line if line.keterangan)
...
class SnapQcLine(models.Model):
    ...
    snap_qc_ids = fields.Many2one('data.inspector')
